...
class UpdateSaleAction
{
    ...
    {
        $storedKtpPath = null;
        $oldKtpPath = $sale->customer?->ktp_file_path;

        try {
            $updatedSale = DB::transaction(function () use ($sale, $data, &$storedKtpPath): Sale {
                $sale = Sale::query()
                    ->with(['vehicle.costs', 'customer', 'payment'])
                    ->lockForUpdate()
                    ->findOrFail($sale->id);

                $this->updateCustomer($sale, $data, $storedKtpPath);

                $snapshots = $this->snapshots($sale->vehicle);
        ...
        if ($storedKtpPath && $oldKtpPath && $oldKtpPath !== $storedKtpPath) {
     ...
     * @param  array<string, mixed>  $data
     */
    private function updateCustomer(Sale $sale, array $data, ?string &$storedKtpPath): void
    {
        $payload = [
        ...
        if (($data['customer_ktp'] ?? null) instanceof UploadedFile) {
            /** @var UploadedFile $ktp */
            $ktp = $data['customer_ktp'];
            $path = $ktp->store("customers/vehicles/{$sale->vehicle_id}/ktp", 'local');
            $storedKtpPath = $path;
